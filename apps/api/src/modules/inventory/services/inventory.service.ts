    ...
    return this.prismaService.$transaction(async (tx) => {
      ...
      if (newStock.greaterThan(maxCapacity)) {
        throw new BadRequestException(
          `La recepción de ${volumeLiters.toString()} L supera la capacidad disponible del tanque '${tank.code}' (Capacidad máxima: ${maxCapacity.toString()} L, Stock actual: ${currentStock.toString()} L).`,
        );
      }
