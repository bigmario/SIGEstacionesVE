  ...
  public async openShift(dto: OpenShiftDto) {
    ...
    if (activeShift) {
      throw new BadRequestException(
        `El vendedor ya tiene un turno activo (ID: ${activeShift.id}). Debe cerrarlo antes de abrir uno nuevo.`,
      );
    }
    ...
    return this.prismaService.$transaction(async (tx) => {
      ...
      for (const item of dto.readings) {
        ...
        if (finalReading.lessThan(initialReading)) {
          throw new BadRequestException(
            `La lectura final (${finalReading}) no puede ser menor a la lectura inicial (${initialReading}) de la bomba ${pump.code}.`,
          );
        }
      ...
      for (const [tankId, totalSoldLiters] of tankSalesMap.entries()) {
        ...
        if (currentStock.lessThan(totalSoldLiters)) {
          throw new UnprocessableEntityException(
            `El inventario del tanque '${tank.code}' (Stock actual: ${currentStock} L) resultaría negativo al descontar ${totalSoldLiters} L vendidos en el turno.`,
          );
        }
